...
class SessionSimulator
  ...
  #@@recomd_mamager = SwayamvaraClient.new
  @@testuser_array =[]
  @@total_users = 3
  @@session_size = 10
  @@total_responses = 0
  ...
  def print_status(scount=@@session_size)
    error_string =  "Repeated Content: #{@@repeat_content_count}\t"
  ...
  def run_session
    ...
    (1..@@session_size).each{|scount|
      ...
      sleep(5)
      cid_list = get_content_id_array(test_user)
      cid_list.each{|cid|
...
SessionSimulator.new(ARGV[0],false).run_session 
